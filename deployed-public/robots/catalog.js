...
function facet(group,labels,key){const host=$(group);host.querySelectorAll('a').forEach(a=>a.remove());for(const [id,label] of [['all','Все'],...Object.entries(labels)]){const b=document.createElement('button');b.type='button';b.dataset.value=id;b.dataset.key=key;b.className='chip';b.textContent=label;b.addEventListener('click',()=>{state[key]=id;update('push')});host.append(b)}}
facet('forms',forms,'form');facet('apps',apps,'app');
document.querySelectorAll('.category-link').forEach(a=>a.addEventListener('click',e=>{e.preventDefault();state[a.dataset.key]=a.dataset.value;writeURL('push','#results');update();$('results').scrollIntoView({block:'start'});$('results').focus({preventScroll:true})}));
...
function update(mode='replace',scroll=false){state.q=q.value;const ranked=records.map(rec=>({...rec,score:rank(rec,state.q)})).filter(rec=>rec.score>=0).sort((a,b)=>b.score-a.score);const visible=ranked.filter(rec=>allowed(rec));const ids=new Set(visible.map(x=>x.r.id));for(const [id,node] of nodes)node.hidden=!ids.has(id);for(const rec of visible)$('cards').append(nodes.get(rec.r.id));
 ...
 document.querySelectorAll('.category-link').forEach(a=>a.setAttribute('aria-current',state[a.dataset.key]===a.dataset.value?'true':'false'));
...
// Categories and directory are static, always-open sections on every viewport.
